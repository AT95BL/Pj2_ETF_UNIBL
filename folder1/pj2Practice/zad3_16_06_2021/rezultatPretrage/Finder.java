...
public class Finder implements FileVisitor<Path> {
    ...
    public void kopirajFajlove() {
        for (var putanjaFajlaKojiKopiramo : this.pronadjeniFajlovi) {
            try {
                Path destinacija = Paths.get(this.ciljniDirektorijum, putanjaFajlaKojiKopiramo.getFileName().toString());
                Files.copy(putanjaFajlaKojiKopiramo, destinacija, StandardCopyOption.REPLACE_EXISTING);
